...
export default function Home() {
  ...
  async function registrarNave(event: any) {
    ...
    try {
      // 1. Subida múltiple al Storage (Bucket: fotos-naves)
      ...
      // 2. Inserción en la tabla 'espacios' (Columnas: imagenes [Array], metros, precio_dia, descripcion)
  ...
  return (
    <main className="p-6 max-w-lg mx-auto bg-slate-50 min-h-screen">
      <div className="bg-white p-8 rounded-3xl shadow-2xl border border-slate-100 mt-10 text-black">
        <h1 className="text-3xl font-black text-indigo-900 mb-1 text-center italic tracking-tighter">VANTAGE BCN</h1>
        <p className="text-slate-400 mb-8 font-bold text-[10px] text-center uppercase tracking-[0.2em]">Captación de Activos Industriales</p>
        
        <form onSubmit={registrarNave} className="space-y-6">
          ...
          <textarea name="descripcion" placeholder="📝 Notas (Altura techos, accesos, luz...)" className="w-full p-4 border-2 border-slate-50 rounded-2xl focus:border-indigo-500 outline-none bg-slate-50 font-bold h-24 text-sm" />
